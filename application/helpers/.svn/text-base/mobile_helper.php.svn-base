 ...
 *
 * @version 0.7.5
 * @author http://detectmobilebrowsers.mobi
 * @package sourcemap
...
function mobile_device_detect($iphone=true,$android=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$mobileredirect=false,$desktopredirect=false) {
	...
	$CI->output->set_header('Cache-Control: no-transform');
	$CI->output->set_header('Vary: User-Agent, Accept');
	if($redirect = ($mobile_browser==true) ? $mobileredirect : $desktopredirect) {
		$CI->output->set_header('Location: '.$redirect);
		exit;
	} else {
