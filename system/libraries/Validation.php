			...
			if (is_array($_POST[$field]) == true) {
				foreach ($_POST[$field] as $field_value) {
						...
						if ($callback === TRUE)
						{
							...
							$result = $this->CI->$rule($field_value, $param);	
							...
							if ( ! method_exists($this, $rule))
							{
								...
								if (function_exists($rule))
								{
									...
									if (isset($this->$field) == true) {
										$temp_array = array($field_value);
										$this->$field = array_merge($this->$field, $temp_array);										
									}
									else {
										$this->$field = array($field_value);
									}
								}								
								continue;
							}

							$result = $this->$rule($field_value, $param);
							var_dump($result);
						}
